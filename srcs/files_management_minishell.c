...
static int	open_file(t_list **new_files, t_list **cmd_new_files,
...
static int	open_file_and_close_prev(t_list **new_files,
		...
		int redir_error_first_position, t_data *d, int is_in_piped)
{
			...
			|| t->position < redir_error_first_position))
	{
		...
		if (t->type == S_IN_REDI
			...
			return (-1 * t->position);
		if (open_file_and_close_prev(&d->new_files, &d->cmd_new_files, \
			t, (*current)->prev) < 0)
			return (-1 * t->position);
		...
		int redir_error_first_position)
{
			...
			|| t->position < redir_error_first_position))
	{
		...
		if (check_out_redir_file(t) < 0)
			return (-1 * t->position);
		if (open_file_and_close_prev(new_files, cmd_new_files, t, \
			(*current)->prev) < 0)
			return (-1 * t->position);
		*current = (*current)->next;
		if (*current)
